...
# project import
from accounts.models import Admin, Notification, RealTor, User, UserType, Visitor
...
class RealTorApiView(ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "messages": "To active Account Veryfi Your Email. Send Email To active Account"
                },
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer.save()
        return Response(
            {"mesage": "User Update Successfully"}, status=status.HTTP_200_OK
        )
...
class ResendEmail(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if not user.exists():
                return Response(
                    {"messages": "This Email Not Registred"},
                    status=status.HTTP_406_NOT_ACCEPTABLE,
                )
            if user[0].is_active:
                return Response(
                    {"messages": "You Are already Active User"},
...
class AcctiveAccount(APIView):
    def post(self, request, uid, token, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"message": "User dose not Exist"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if new_password:
            ...
            if user.check_password(new_password):
                return Response(
                    {
                        "message": "You have given new password same as the old one. Give new password"
                    },
...
class RequestSearchAPIView(ListAPIView):
    ...
    )  # ('id','first_name','last_name','create','update','real_estate__user')
